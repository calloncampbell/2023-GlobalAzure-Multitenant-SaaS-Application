...
resource database 'Microsoft.Sql/servers/databases@2022-08-01-preview' = {
  name: envSlot == '' ? '${databaseServerName}/gab-${envType}-${locationShort}-saas-elstic-job-sqldb' : '${databaseServerName}/gab-${envSlot}-${locationShort}-saas-elastic-job-sqldb'
  location: location
  sku: {
    name: sqldbSkuName
    tier: sqldbSkuTier
...
output elasticJobDatabaseId string = database.id
